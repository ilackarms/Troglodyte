...
public class ProjectileCollisionBehaviour : MonoBehaviour
{
  ...
  private void CollisionEnter()
  {
    ...
    if (SendCollisionMessage) {
		EffectSettings effectSettings = GetComponentInParent<EffectSettings> ();
		Spell spell=null;
		if(effectSettings!=null) spell = effectSettings.spell;
		Debug.Log ("Hitfukkk!!!!!");
		if(!(hit.transform==null || spell==null)){
			spell.notifyCollision(hit);
		}
